    ...
    return (
        <div className={' w-full flex flex-col gap-2.5 items-center px-12 pb-12 relative '}>
            <div className={' absolute right-0'}>
                <button onClick={() => clearCurrentModal()} className={'bg-base p-3 rounded-xl'}><ExitBtnIcon/></button>
            </div>
            <div className={'p-2'}></div>
            <p className={'font-semibold  text-title text-3xl pb-1'}>Измените права доступа пользователя</p>
            <Select menu={{selectable: true, activeSelect: user.permissions,  placeholder: 'Выберите права доступа', items: items}} setSelectItem={setAccessSelect} selectItem={accessSelect}/>
            <Button onClick={handleSendEmail} type={'big-success'}>Сохранить изменения</Button>
        </div>
    )
}