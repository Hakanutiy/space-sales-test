...
export const TeamList = ({data}) => {
    ...
    if (data) return <>{data.map(team => (
        <section key={team.id} className={'flex w-full gap-3'}>
            <img className={'rounded-full w-16 h-16 object-cover'} src={team.image || '/img/defaultAvatar.svg'} alt={team.name}/>
            <main className={'w-full flex flex-col justify-center'}>
                <div className={'flex justify-between items-center'}>
                    ...
                    <DropDown menu={{items: items, selectable: true, key: team.id}} setSelectItem={setActionUser}>
                        <MoreIcon/>
                    </DropDown>
                ...
                <div className={'flex gap-2 flex-wrap'}>
                    {team.permissions.map((permission, index) => (
                        <div key={index}
                             className={'border-desc border py-0.5 px-3 rounded-xl text-desc '}>{permission}</div>
                    ))}
                </div>
            </main>
            <MainModal modalId={'sendInvite'}><SendIvite/></MainModal>
            <MainModal modalId={'editTeamUser'}><EditTeamModal user={actionUser.length > 0 && currentUser}/></MainModal>
            <MainModal modalId={'sendCodeTeam'}><SendCodeModal/></MainModal>
            <MainModal modalId={'deleteTeamUser'}><DeleteTeamModal userId={ actionUser.length > 0 && actionUser[0].key}/></MainModal>
        </section>
    ))}
        </>
    else return  <Spinner/>
